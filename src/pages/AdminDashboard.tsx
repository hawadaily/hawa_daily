...
އާމިން`;
  ...
  const handleDeleteChildrenStory = async (storyId: string) => {
    ...
    try {
      // Fetch story to get image URL
      const storyDoc = await getDoc(doc(realStoryDb, 'real-stories', storyId));
      if (storyDoc.exists()) {
        const storyData = storyDoc.data();
        if (storyData?.coverImage) {
          try {
      ...
      // Delete all episodes
      const episodesSnapshot = await getDocs(collection(realStoryDb, 'real-stories', storyId, 'episodes'));
      for (const episodeDoc of episodesSnapshot.docs) {
        await deleteDoc(doc(realStoryDb, 'real-stories', storyId, 'episodes', episodeDoc.id));
      }
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-slate-50 p-6">
                    ...
                    <div className="space-y-6">
                      ...
                      <h1 className="text-2xl font-bold leading-[2.5] text-[#0077b6]">{titleDv}</h1>
                      <p className="text-sm leading-7 text-[#00b4d8]">{excerptDv}</p>
                      <div className="space-y-6">
                        {(() => {
                          const bodyText = body;
